...
import threading
stopuser = {}
token = '7263904870:AAH1l-T_x3-j6iSqixDXd7Q-H3URq9l-kYU'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			fady = types.InlineKeyboardButton(text="‌ 𝗢𝗪𝗡𝗘𝗥  ‌", url="https://t.me/V_1_V_V")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="‌ 𝗢𝗪𝗡𝗘𝗥 ‌", url="https://t.me/V_1_V_V")
...
𝘽𝙤𝙩 𝘽𝙮 @V_1_V_V''', reply_markup=mes)
...
def respond_to_vbv(message):
  ...
  "timer": "none",
	...
	if BL == '𝗙𝗥𝗘𝗘':
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="‌ 𝗢𝗪𝗡𝗘𝗥  ‌", url="@V_1_V_V")
		fady = types.InlineKeyboardButton(text="‌ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇  ‌", url="https://t.me/far3on_spam")
		keyboard.add(contact_button, fady)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='FAR3ON-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
